...
describe ThorAddons::Helpers::OptionType do
  describe ".valid?" do
    it "should return true if the option type is correct" do
      ...
      expect(
        described_class.new("{ \"foo\" => 1 }", :hash).valid?
      ).to eq(false)
  ...
  describe ".convert_string" do
    it "should return the option with the correct type" do
      ...
      expect(
        described_class.new("a b", :array).convert_string
      ).to eq(%w[a b])

      expect(
        described_class.new("a:b", :hash).convert_string
      ).to eq("a" => "b")

      expect(described_class.new("1", :numeric).convert_string).to eq(1)
      expect(
        described_class.new("22.5", :numeric).convert_string
      ).to eq(22.5)
    end
  end
end
