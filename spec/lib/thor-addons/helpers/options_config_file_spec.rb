    ...
    describe OptionsConfigFile do
      ...
      describe "#extract_command_data" do
        it "should extract the data from th given hash" do
          expect(described_class.extract_command_data(data, "cmd_1")).to eq(
            { "foo" => "bar", "biz" => true }
          )
        end

        it "should extract only the global if the command is not present" do
          expect(described_class.extract_command_data(data, "cmd_3")).to eq(
            { "foo" => "bar" }
          )
        end
      ...
      describe "#parse_command_config_data" do
        context "with invocations" do
          it "should parse the config file" do
            expect(
              described_class.parse_command_config_data(
                data,
                invocations,
                "cmd_2"
              )
        ...
        context "with no invocations" do
          it "should parse the config file" do
            expect(
              described_class.parse_command_config_data(data, nil, "cmd_1")
